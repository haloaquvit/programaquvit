...
CREATE POLICY IF NOT EXISTS "Authenticated users can view account transfers" 
ON public.account_transfers FOR SELECT 
USING (auth.role() = 'authenticated');

-- Create policy for authenticated users to insert transfers
CREATE POLICY IF NOT EXISTS "Authenticated users can create account transfers" 
...
CREATE INDEX IF NOT EXISTS idx_account_transfers_created_at ON public.account_transfers(created_at DESC);
